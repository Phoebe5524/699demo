...
const CATEGORIES_CONFIG = {
    'Food': { icon: <Utensils />, color: 'bg-[#BADA85]/60', textColor: 'text-[#d48888]', label: 'Food', type: 'Flexible' }, 
    'Transportation': { icon: <Car />, color: 'bg-[#7D85F2]/60', textColor: 'text-[#7abdc5]', label: 'Trans', type: 'Flexible' }, 
    'Shopping': { icon: <ShoppingCart />, color: 'bg-[#fcf373]/60', textColor: 'text-[#a88baec]', label: 'Shop', type: 'Flexible' }, 
    'Entertainment': { icon: <Clapperboard />, color: 'bg-[#7D5CF1]/60', textColor: 'text-[#d6966d]', label: 'Fun', type: 'Flexible' }, 
    'Personal Care': { icon: <Sparkles />, color: 'bg-[#F24F50]/60', textColor: 'text-[#8bcdd6]', label: 'Care', type: 'Flexible' }, 
    'Education': { icon: <GraduationCap />, color: 'bg-[#FCC773]/60', textColor: 'text-[#8e6fa0]', label: 'Edu', type: 'Fixed' }, 
    'Savings': { icon: <PiggyBank />, color: 'bg-[#7DBFF2]/60', textColor: 'text-[#96a37e]', label: 'Save', type: 'Fixed' }, 
    'Subscription': { icon: <Mail />, color: 'bg-[#F27DDC]/60', textColor: 'text-[#c9c568]', label: 'Sub', type: 'Fixed' }, 
    'Misc': { icon: <Zap />, color: 'bg-[#FC7373]/60', textColor: 'text-[#888888]', label: 'Misc', type: 'Flexible' }, 
    'Income': { icon: <Plus />, color: 'bg-gray-800', textColor: 'text-white', label: 'Inc', type: 'Fixed' }, 
  };
...
export default function Home() {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    // 2. Spending Type Filter
    if (spendingType !== 'ALL') {
      data = data.filter(t => (t.type || 'flexible').toLowerCase() === spendingType.toLowerCase());
    } else {
  ...
  const toggleEnvelope = (cat) => {
    if (selectedEnvelopes.includes(cat)) setSelectedEnvelopes(prev => prev.filter(c => c !== cat));
    else if (selectedEnvelopes.length < 5) setSelectedEnvelopes(prev => [...prev, cat]);
  };

  const toggleUnit = () => {
    const options = [50, 100, 200];
    setUnit(options[(options.indexOf(unit) + 1) % options.length]);
  };
  ...
  const handleDragMove = (e) => {
    ...
    const calculatedAmount = (Math.abs(deltaX) / 200) * unit;
  ...
  const handleDragEnd = () => {
    if (isDragging && dragAmount > 0) handleAddClick(parseFloat(dragAmount.toFixed(2)));
    setIsDragging(false); setDragPhase('start'); setDragAmount(0);
  };
  ...
  const handleSaveTransaction = () => {
    ...
    // 自动判断 Type
    const config = CATEGORIES_CONFIG[selectedCategory];
    const newType = config ? (config.type || 'Flexible') : 'Flexible';
  ...
  const displayBlocks = useMemo(() => {
    ...
    filteredTransactions.forEach(t => {
      ...
      while (cellsNeeded > 0.01) {
        ...
        if (cellsToTake > 0) blocks.push({ ...t, uniqueKey: `${t.id}-${blocks.length}`, widthPercent: (cellsToTake / CELLS_PER_ROW) * 100, isStart: blocks.length === 0 || blocks[blocks.length - 1].id !== t.id, color: CATEGORIES_CONFIG[t.category]?.color || 'bg-gray-400' });
  ...
  return (
    <div className="h-full flex flex-col relative bg-white">
        <header className="px-6 pt-12 pb-4 bg-white z-50 relative">
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-baseline gap-1 cursor-pointer">
                <h1 className="text-3xl font-bold text-gray-900 tracking-tight">{getMonthName(currentDate)}</h1>
              </div>
              <button onClick={() => changeMonth(1)}><ChevronRight size={20} className="text-gray-400"/></button>
            </div>
            <div className="flex gap-2">
               <button onClick={() => setShowViewMenu(!showViewMenu)} className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 hover:bg-gray-200 transition-colors"><LayoutGrid size={20}/></button>
          ...
          {showViewMenu && <div className="absolute right-6 top-24 bg-white rounded-xl p-1 shadow-2xl flex flex-col gap-1 z-50 w-36 animate-slide-up border border-gray-100">{['Biweekly', 'Weekly', 'Daily'].map(m => (<button key={m} onClick={() => {setViewMode(m); setShowViewMenu(false)}} className="px-3 py-2.5 text-sm font-medium text-left hover:bg-gray-50 rounded-lg flex justify-between items-center text-gray-700">{m}</button>))}</div>}
        </header>
            ...
            <div className="flex justify-between items-center animate-slide-up">
              <div className="flex gap-2 flex-1 mr-3">
                {['ALL', 'Fixed', 'Flexible'].map(type => (
                  <button key={type} onClick={() => setSpendingType(type)} className={cn("flex-1 py-3 rounded-2xl text-sm font-bold transition-all flex items-center justify-center gap-1.5", spendingType === type ? "bg-[#333] text-white shadow-md" : "bg-gray-100 text-gray-500 hover:bg-gray-200")}>
                    {type === 'ALL' && 'ALL'}
              ...
              <button onClick={toggleUnit} className="bg-[#F2F2F7] px-4 py-3 rounded-2xl text-sm font-bold text-[#543CCB] hover:bg-gray-200 min-w-[70px] flex justify-between items-center">{unit} <ChevronDown size={14}/></button>
            </div>
          ) : (
            <div className={cn("absolute inset-x-6 top-0 bottom-2 rounded-2xl flex items-center justify-between px-4 font-bold text-sm shadow-sm transition-colors", dragPhase === 'adding' ? "bg-yellow-200 text-yellow-900 border border-yellow-400" : dragPhase === 'reducing' ? "bg-blue-100 text-blue-900 border border-blue-300" : "bg-yellow-50 text-yellow-800 border border-yellow-200")}>
               <div className="flex items-center gap-2">{dragPhase === 'start' && <span>Drag to start</span>}{dragPhase === 'adding' && <span className="bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded">Adding</span>}{dragPhase === 'reducing' && <span className="bg-blue-500 text-white text-[10px] px-1.5 py-0.5 rounded">Reducing</span>}{dragPhase !== 'start' && <span>Roughly ${dragAmount.toFixed(2)}</span>}</div><div className="flex gap-1"><MoveHorizontal size={16}/></div>
            </div>
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-white relative" ref={gridRef}>
          <div className="min-h-full relative pb-24 flex">
            <div onClick={() => navigate('/analysis')} className="w-4 mr-4 bg-gray-100/50 hover:bg-gray-200/50 rounded-r-xl my-4 ml-0 cursor-pointer flex items-center justify-center group"><ChevronLeft size={16} className="text-gray-300 group-hover:text-gray-500"/></div>
            <div className="flex-1 relative">
                <div className="absolute inset-0 pointer-events-none z-1 border-r border-[#C7C7CC]" style={{ backgroundSize: `20% ${ROW_HEIGHT}px, 5% ${ROW_HEIGHT}px, 100% ${ROW_HEIGHT}px`, backgroundImage: `linear-gradient(to right, #A29FAB 1px, transparent 1px), linear-gradient(to right, #E3E2E8 1px, transparent 0.5px), linear-gradient(to bottom, #A29FAB 1px, transparent 1px)` }}></div>
                <div className="absolute -right-6 top-0 bottom-0 flex flex-col items-center pointer-events-none z-10">
                   {Array.from({ length: totalRowsNeeded }).map((_, i) => (<div key={i} className="flex items-end justify-end pr-1 text-[9px] text-gray-400 font-bold" style={{ height: ROW_HEIGHT }}>{(i + 1) * unit}</div>))}
                </div>
                <div className="relative flex flex-wrap content-start pt-[1px]">
                  {displayBlocks.map((block) => (
                    <div key={block.uniqueKey} onClick={() => !isDragging && handleEditClick(block)} className={cn("h-[68px] relative transition-all hover:brightness-105 overflow-hidden cursor-pointer", block.color)} style={{ width: `${block.widthPercent}%` }}>
                      {block.isStart && block.widthPercent > 12 && (<div className="absolute inset-0 z-30 pointer-events-none p-1.5"><span className="absolute top-1.5 left-1.5 text-[9px] font-bold text-black leading-none opacity-80">{formatDate(block.date)}</span><div className="absolute top-1.5 right-1.5 text-black opacity-80">{React.cloneElement(CATEGORIES_CONFIG[block.category].icon, { size: 12 })}</div><span className="absolute bottom-1.5 left-1.5 text-[11px] font-extrabold text-black leading-none opacity-90">${block.amount}</span></div>)}
                    </div>
                  ))}
                  <div onMouseDown={handleDragStart} onTouchStart={handleDragStart} className={cn("h-[68px] flex flex-col items-center justify-center text-gray-500 text-xs cursor-pointer border border-dashed border-gray-300 transition-colors z-30", isDragging ? "bg-yellow-100 border-yellow-400" : "bg-gray-50 hover:bg-gray-100")} style={{ width: isDragging ? `${Math.max(20, (dragAmount/unit)*20*5)}%` : '20%', minWidth: '20%' }}>
                    {!isDragging && <Plus size={16} className="mb-1"/>}<span className="text-[10px] font-bold">{isDragging ? 'Insert...' : 'Insert'}</span>
                  </div>
                </div>
            </div>
            <div onClick={() => navigate('/envelope')} className="w-4 ml-8 bg-gray-100/50 hover:bg-gray-200/50 rounded-l-xl my-4 mr-0 cursor-pointer flex items-center justify-center group"><ChevronRight size={16} className="text-gray-300 group-hover:text-gray-500"/></div>
          </div>
        </div>

        {/* Envelope Filter Modal */}
        {showEnvelopeModal && (
          <div className="absolute inset-0 z-50 flex flex-col justify-end">
            ...
            <div className="bg-white rounded-t-[30px] p-6 animate-slide-up shadow-2xl relative h-[60%] flex flex-col">
               <div className="flex justify-between items-start mb-2">
                 <div><h2 className="text-xl font-bold text-gray-900">Filter by Envelope</h2><p className="text-xs text-gray-400">Subtitle here type it</p></div>
                  ...
                  {Object.keys(CATEGORIES_CONFIG).filter(k => k !== 'Income').map(cat => {
                    ...
                    return (
                      <button key={cat} onClick={() => toggleEnvelope(cat)} className={cn("flex items-center justify-between px-4 py-3 rounded-2xl border transition-all", isSelected ? `${config.color} border-transparent` : "bg-white border-gray-100 hover:border-gray-300")}>
                         <div className="flex items-center gap-2">
                            <div className={cn("w-6 h-6 rounded-full flex items-center justify-center text-xs", isSelected ? "bg-white/50 text-black" : "bg-gray-100 text-gray-500")}>{React.cloneElement(config.icon, { size: 14 })}</div>
        ...
        {/* Add/Edit Transaction Modal (Fully Restored) */}
        {showEditModal && (
          <div className="absolute inset-0 z-50 flex flex-col justify-end">
